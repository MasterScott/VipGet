...
class Helper
{
    ...
    public static function isValidAccountNumber($acct) {
        return preg_match("/^(U|X)[0-9]{1,}$/", $acct);
    }

    public static function request_post($url, $data, $optional_headers = null)
    {
    ...
    public static function GetLangContent($langPath = '')
    {
        $lang_content = array();
        $langFile = $langPath . '.xml';
    ...
    /*
     * Class cho ứng dụng vipget.net
     */
    public static function size_name($link,$cookie){
        set_time_limit(0);
        ...
        if(stristr($header,"TTP/1.0 200 OK") || stristr($header,"TTP/1.1 200 OK") || stristr($header,"TTP/1.1 206"))
            $filesize = trim (self::cut_str ( $header, "Content-Length:", "\n" ) );
        else $filesize = -1;
        if(!is_numeric($filesize)) $filesize = -1;
        $filename = "";
        if(stristr($header,"filename")) {
            $filename = trim (self::cut_str ( $header, "filename", "\n" ) );
        }
    ...
    static function convert_name($filename){
        $filename =urldecode($filename);
        $filename =self::uft8html2utf8($filename);
        $filename = preg_replace("/(\]|\[|\@|\"\;\?\=|\"|=|\*|UTF-8|\')/","",$filename);
        $filename = preg_replace("/(HTTP|http|WWW|www|\.html|\.htm)/i","",$filename);
        return $filename;
    }
    static function uft8html2utf8( $s ) {
    ...
    public static function curl($url,$cookies,$post,$header=1,$redirect=0){
        set_time_limit(0);
    ...
    static function cut_str($str, $left, $right)  {
    ...
    public static function GetCookies($content){
        preg_match_all('/Set-Cookie: (.*);/U',$content,$temp);
        $cookie = $temp[1];
        $cookies = implode('; ',$cookie);
        return $cookies;
    }
    public static function GetAllCookies($page) {
        $lines = explode("\n", $page);
        $retCookie = "";
    ...
    public static function GetLocation($content) {
    ...
    public static function getCookie($site) {
        global $db;

        if (!empty($site)) {
            $sql = $db->sql_query("SELECT * FROM cookie WHERE site='" . $site . "'");
    ...
    public static function saveCookie($site, $value) {
        global $db;

        if (!empty($site) && !empty($value)) {

            $sql = $db->sql_query("SELECT * FROM cookie WHERE `site`='".$site."'");
            if ($db->sql_affectedrows() > 0) {
                $sql = $db->sql_query("UPDATE cookie SET `value`='".$value."' WHERE `site`='".$site."'");
                if ($db->sql_affectedrows() > 0)
                    return true;
            } else {
                $sql = $db->sql_query("INSERT INTO cookie (`site`,`value`) VALUES ('".$site."', '".$value."') ON DUPLICATE KEY UPDATE `site`='".$site."',`value`='".$value."'");
                if ($db->sql_affectedrows() > 0)
                    return true;
            }
        }
    ...
    public static function getAccount($site) {
        global $db;

        if (!empty($site)) {
            $sql = $db->sql_query("SELECT * FROM account WHERE `site`='" . $site . "'");
            if ($db->sql_affectedrows() > 0) {
                $row = $db->sql_fetchrow($sql);
    ...
    public static function fixUrl($u) {
